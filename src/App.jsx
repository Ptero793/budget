      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2 min-w-0">
            ...
            {/* Hide the wordmark on phones so the month selector + sign-out fit. */}
            <h1 className="text-lg font-bold text-gray-900 whitespace-nowrap hidden sm:block">Budget Tracker</h1>
          </div>
          <div className="flex items-center gap-2 sm:gap-3">
            <MonthSelector />
            <button
              onClick={onSignOut}
              className="text-xs text-gray-500 hover:text-gray-700 whitespace-nowrap px-1 py-1"
            >
              Sign out
            </button>
          </div>
          ...
          <div className="flex gap-0.5 overflow-x-auto scrollbar-none">
            {TABS.map(tab => (
              <button
                ...
                className={`px-4 sm:px-5 py-3 text-sm font-medium border-b-2 whitespace-nowrap transition-colors ${
                  activeTab === tab.id
